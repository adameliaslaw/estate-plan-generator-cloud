...
/** Days between today (midnight local) and an ISO date string. Negative = overdue. */
...
export default function DashboardPage() {
  ...
  // Upcoming deadlines across all clients, sorted by date ascending.
  // Includes overdue items (date in the past) so they surface prominently.
  const upcomingDeadlines = useMemo(() => {
    const rows: Array<{ client: Client; deadline: ClientDeadline }> = [];
    for (const c of allClients) {
      if (c.isArchived) continue;
      const deadlines = c.deadlines ?? [];
      for (const d of deadlines) {
        if (d.completed) continue;
        rows.push({ client: c, deadline: d });
      }
    }
    rows.sort((a, b) => a.deadline.date.localeCompare(b.deadline.date));
    return rows;
  }, [allClients]);
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-semibold text-[#1a365d]">Upcoming Deadlines</h3>
            <span className="rounded-full bg-indigo-100 px-2 py-0.5 text-xs font-semibold text-indigo-700">
              {upcomingDeadlines.length}
            </span>
          </div>
          <span className="text-xs text-gray-400">Signings · filings · follow-ups</span>
        </div>
        {upcomingDeadlines.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-2 px-5 py-10 text-center">
            <CalendarClock className="h-8 w-8 text-gray-300" />
            <p className="text-sm text-gray-400">No deadlines scheduled</p>
          </div>
        ) : (
          <ul className="divide-y divide-gray-100">
            {upcomingDeadlines.slice(0, 8).map(({ client, deadline }) => {
              const diff = daysUntil(deadline.date);
              ...
              return (
                <li
                  key={`${client.id}-${deadline.id}`}
                  onClick={() => navigate(ROUTES.CLIENT_DETAIL(client.id))}
                  className="flex cursor-pointer items-center justify-between gap-3 px-5 py-3 hover:bg-indigo-50/40 transition-colors"
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <p className="truncate text-sm font-medium text-[#1a365d]">
                      ...
                      <span className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-gray-500">
                        {DEADLINE_TYPE_LABEL[deadline.type] ?? deadline.type}
                      </span>
                    </div>
            ...
            {upcomingDeadlines.length > 8 && (
              <li className="px-5 py-2 text-center text-xs text-gray-400">
                +{upcomingDeadlines.length - 8} more
              </li>
            )}
          </ul>
        )}
