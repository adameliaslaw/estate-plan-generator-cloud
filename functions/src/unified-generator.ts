...
export async function generateDocument(
  ...
  if (params.spouseRole === 'spouse' && clientData.spouseInfo) {
    console.log(`[unifiedGenerator] Spouse swap for ${docType}: ${(clientData.spouseInfo as Record<string, unknown>).firstName ?? 'unknown'} ↔ ${(clientData.personalInfo as Record<string, unknown> | undefined)?.firstName ?? 'unknown'}`);
    // Extracted to spouse-swap.ts (behavior-identical) so other generation
    // paths — e.g. high-fidelity .docx package fills — can reuse the swap.
    clientData = swapClientDataForSpouse(clientData);
    if (clientContext) swapClientContextForSpouse(clientContext);
  }
