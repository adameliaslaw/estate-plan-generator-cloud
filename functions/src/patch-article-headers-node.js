 ...
 * Run this locally via: node patch-article-headers-node.js
 */
...
async function patchTemplates() {
  ...
  for (const doc of templatesSnap.docs) {
    ...
    let articleIndex = 0;
    const patchedHtml = originalHtml.replace(/<p class="tr-art1">(.*?)<\/p>/gi, (match, content) => {
    ...
    if (patchedHtml !== originalHtml) {
      console.log(`[Patch] Updating ${doc.id} (Name: ${data.name}) with corrected headers...`);
      await doc.ref.update({ content: patchedHtml, updatedAt: admin.firestore.FieldValue.serverTimestamp() });
    } else {
      console.log(`[Patch] No changes needed for ${doc.id} (Name: ${data.name}).`);
    }
  }
