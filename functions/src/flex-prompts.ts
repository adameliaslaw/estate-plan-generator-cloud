/**
 * functions/src/flex-prompts.ts
 *
 * Flex document system prompts and AI generation logic.
 * Extracted from generate-flex-document.ts into its own module so that
 * the unified generator can import it without circular dependencies.
 *
 * Handles ancillary/supplementary document types:
...
export function buildFlexSystemPrompt(docType: string, customPrompt?: string): string {
  const baseContext = `
...
/**
 * Generate a flex/ancillary document using AI with a doc-type-specific system prompt.
 * This function extracts client context from the raw client data and calls the LLM.
 */
export async function generateFlexAI(params: FlexGenerateParams): Promise<GeneratedDoc> {
  const { docType, clientData, firmData, customPrompt, additionalData } = params;

  const safeClient = sanitizeObject(clientData);
  const safeFirm = sanitizeObject(firmData);

  const pi = (safeClient as admin.firestore.DocumentData).personalInfo ?? {};
  const clientFullName = [pi.firstName, pi.middleName, pi.lastName, pi.suffix]
    .filter(Boolean)
    .join(' ');

  const packageType = (safeClient as admin.firestore.DocumentData).packageDetails?.packageType ?? 'foundation';
  const packageFee = (safeClient as admin.firestore.DocumentData).packageDetails?.estimatedFee;
  const trusts: admin.firestore.DocumentData[] = (safeClient as admin.firestore.DocumentData).trusts ?? [];
  const fiduciaries = (safeClient as admin.firestore.DocumentData).fiduciaries ?? {};
  const executor = fiduciaries.executor ?? {};
  const trustee = fiduciaries.trustee ?? {};
  const distribution = (safeClient as admin.firestore.DocumentData).distribution ?? {};
...
`.trim();
  ...
  const raw = await callAI(systemPrompt, userPrompt, firmData, {
    model: (firmData as Record<string, unknown>).documentDraftingModel as string ?? 'claude-opus-5',
  ...
  try {
    ...
    console.error(`[flexPrompts] Parse error for ${docType}:`, err);
    throw new Error(
      `Failed to parse generated flex document: ${err instanceof Error ? err.message : 'Unknown error'}`,
    );
  }
}
