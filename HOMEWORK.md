...
**🟢 R5-037 — PR #121 OPEN, AWAITING ADAM (emulator verification + Never-Break merge sign-off).** Implemented the systemic firm-scoping Adam approved: added `isFirmAdmin(firmId) = isAdmin() && belongsToFirm(firmId)`; replaced all 43 firm-scoped `if isAdmin()` grants → 38 `isFirmAdmin(firmId)` (firm-path collections) + 3 `isFirmAdmin(resource.data.firmId)` (the notes/payments/documents collection-group reads) + left 2 bare `isAdmin()` on the global single-tenant pipeline collections (`pipeline_state`, `pipeline_audit_log` — not all docs carry a firmId field, so `resource.data.firmId` scoping would break legit admin monitoring; residual noted). `canManage*()` helper internals untouched (already firm-scoped via `&& belongsToFirm` at call sites). Header comment updated. Structure test (`security-rules.test.ts`) updated + R5-037 regression assertions (pendingTranscripts firm-scoped; exactly 2 bare `if isAdmin()` survive). Verified: 650/650 unit tests, eslint, full manual diff review, static scope-check (no `isFirmAdmin(firmId)` in any collection-group scope).
- **⚠️ EMULATOR GAP:** the behavioral rules emulator test could NOT run in the agent environment — the Firestore emulator needs **Java** (not installed here), and `@firebase/rules-unit-testing` isn't a project dep. Before merge, run on a Java-equipped machine: `firebase emulators:exec --only firestore "npm run test"` (or a rules-unit-testing behavioral spec). The rules also compile server-side on CI deploy (catches syntax errors, not logic). **Adam: review the diff + run the emulator, or sign off knowing the transformation is mechanical + statically verified. NOT auto-merged.**
- **Behavioral checklist for the emulator run:** (a) admin of firm A CAN read/write firm A clients/documents/notes/payments/transcripts; (b) admin of firm B CANNOT read firm A's; (c) attorney/paralegal/client access unchanged; (d) collection-group notes/payments/documents reads still work for in-firm staff; (e) an admin can still read/delete their OWN firm doc.
