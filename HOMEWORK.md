...
|---|---|---|
| **#162** SA key in git history | Headline key `c059f6a5…` **provably dead** (7/27 GCP-verified session record below); history never rewritten (blob inert, marked optional); two stray user-managed keys still parked for Adam; **no secret scanner or push protection exists anywhere** — that half was tracked nowhere. | **Adam:** confirm the 7/27 record closes the headline; delete the two stray keys (parked item). **Agent:** gitleaks CI step **done — PR open 2026-08-07 awaiting Adam's sign-off** (CI-workflow class): pinned checksum-verified binary, full-history scan verified green locally against a `.gitleaks.toml` allowlist (public Firebase web key + test fixture) and `.gitleaksignore` (11 dead historical fingerprints, each annotated with rotation evidence); planted-secret fail path verified exit 1. **Adam:** enable GitHub push protection. |
