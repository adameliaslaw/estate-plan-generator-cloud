...
1. **Smoke test — RETRY now (root cause fixed).** Earlier "Test Connection" returned *"SendGrid API key not configured."* **Root cause was NOT the data** — the migration is fine (secrets doc holds `sendGridApiKey`, verified read-only). The live `testSendGridConnection` + every email sender were frozen on **2026-06-25 revisions** (pre-AR), so their bundled `getFirmData` had no `loadFirmSecrets` merge and looked for the raw key the migration deleted. Cause: the recurring **409 deploy storm** (see below) silently stranded them across the AR + #62 deploys while CI still reported green. **Fixed 2026-06-29: force-redeployed all stale functions from current `main` in small batches.** → **Re-click Test Connection; it should now verify green.** If it still errors, escalate (data + code are both confirmed correct).
