...
**✅ Shipped #6 (#75, merged `ac4fdc8`):** DA — removed the dead "Regenerate with updated data" RowActions path in DocumentVault (never wired; editor already has Regenerate — Adam chose remove). Kept `DocumentStatusBadge`'s live `isStale` warning. Ledger DA → fixed. **All identified frontend audit cleanups are now closed except CU (e-sign, planned/blocked on Adam's Dropbox Sign key).**
