...
**🟡 #2 PARTIAL — Storage provisioned, but CI auto-deploy of storage.rules STILL blocked by a deployer-SA gap.** Adam provisioned Firebase Storage (console "Get Started" gone). Verified as owner: `defaultBucket` now resolves HTTP 200 (was unregistered on 6/15), the bucket is registered, and the live storage ruleset is **byte-identical** (matching MD5) to repo `storage.rules` — zero drift; `firebase deploy --only storage` succeeds cleanly **as an owner (adamelias66)**.
- Re-added `storage` to CI via PR #34 (`2a15861`, merged) — but the deploy **FAILED** (run `27618316599`): under the CI deployer SA it STILL errors `"Firebase Storage has not been set up"` and aborts the WHOLE deploy (same blast radius as the 6/11 outage). **Reverted** (`957bb60`) to restore green functions+firestore deploys. storage.rules stays manual-deploy (fine — rules rarely change, live==repo).
- **Diagnosis so far:** NOT a missing storage permission — the SA `github-action-1189038360` already has `roles/firebasestorage.admin`, which includes `firebasestorage.defaultBucket.get` + `buckets.get/list`. Couldn't impersonate the SA to test directly (no `iam.serviceAccountTokenCreator`). Leading hypotheses for next session: (a) CI firebase-tools version checks storage-setup via `adminSdkConfig` (`firebase.googleapis.com`), which needs **`firebase.projects.get`** — a permission NONE of the SA's roles grant (it has the four narrow `firebase*.admin` roles but no general `firebase.viewer`/`firebase.admin`); try granting `roles/firebase.viewer` to the SA, then re-add storage and test ONE CI cycle. (b) CI identity differs from local. **Don't guess-and-deploy** — verify the hypothesis in CI before re-adding storage. Low priority.

**Net 2026-06-16:** #1 Calendar OAuth fixed at the root (published to production) → durability-watch until ~6/30. #2 Storage provisioned + rules in sync + manual deploy working; only the CI *auto*-deploy of storage.rules remains, blocked by the SA gap above (low priority). Standing watch-items: OAuth durability + storage-rules CI re-add.
