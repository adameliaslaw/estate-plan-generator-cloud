...
| ~~**D2**~~ | ~~**Wire `ApportionmentMode` into it.**~~ **✅ DONE 2026-08-08 (#320)** — generation reads `interviewSettings/current → trust.apportionmentMode` via the `interview-settings.ts` bridge; precedence interview > legacy firm field > 'hybrid' code default. | Closed PR #280's loose end: the mode is now a firm default the Settings → Interview screen controls, which was the point — theirs was the right *shape*, ours keeps the three-mode statutory substance (54:35-6 vs 3B:24-1). |
...
| # | Item | Notes |
|---|------|-------|
| ~~**G1**~~ | ~~**Official filled PDFs for IT-EXT and the L-9.**~~ **✅ ALREADY SHIPPED — this row was stale. Verified 2026-08-06.** | Both exist and are wired. `fillITEXTPdf` (`forms/it-ext-pdf.ts`) fills the State's `itext.pdf`; `fillL9Pdf` (`forms/l9-pdf.ts`) fills `itl9.pdf`; both blanks are committed under `functions/assets/` alongside `itr-blank.pdf`. Wired at `inheritance-tax-review.ts:371` and `:388`. **The "renders as HTML workpaper" premise was wrong** — the HTML is returned *alongside* the PDF, not instead of it, and the PDF is opt-in per request (`body.pdf === true`) purely to keep a few hundred KB off the wire when the on-screen workpaper is all that's wanted. That is the same opt-in the IT-R already used. IT-Estate remains correctly unbuilt and the code says why: the State prints a different Estate Tax return either side of 2017-01-01 and neither is mapped — consistent with pre-2018 being declined. `fillL9Pdf` also deliberately refuses an L-9(A) and surfaces `failed-precondition` with its reason, for the same pre-2018 reason. |
| **G2** | **The wills Drive pipeline — repair or retire.** **✅ INVESTIGATION DONE 2026-08-06. It is not broken. The scope call is Adam's and is now a real question rather than a diagnosis.** | **Why it never started, named:** it is **fail-closed by design and was never armed.** `willsStartBackfill`, `willsPilotRun` and `willsProcessor` all gate on `pipeline_state/control` carrying **`firmId`** (else *"Pipeline owner firm is not configured"*) and **`enabled: true`** (else *"Pipeline is disabled"*). **No code anywhere writes either field** — every occurrence in the tree is a comment or an error string. The only writer of `control` is `willsSetupDriveWatch`, which merges in `webhook_url` alone. So `pipeline_state=0` and `wills_documents=0` are the *correct* state of an unarmed pipeline, not evidence of a fault. The code is deployed: all four callables are exported from `index.ts`. **Repair costs two fields in the Firestore console, not a code change.** <br><br>**But the question has moved on.** The clause-miner walks the **identical** Drive root (`1TuJOw7hy4xKm6EJeyFb5IYS4I6eoVk-j`, pinned in both `wills-backfill.ts` and the clause-miner workflow) and its `extract` stage already produces the same fact vocabulary — `trustStructures`, `distributionStandard`, `fundedStatus`, `estateSizeBand` — matching `wills-schema.ts`'s `TRUST_STRUCTURES` / `DISTRIBUTION_STANDARDS` / `FundedStatus`. **They are not duplicates, though:** clause-mining is trust-clause-family focused (`docType: 'trust'`), while the wills pipeline produces *per-document* classification the catalog has no equivalent for — document type, file format, firm origin (predecessor vs current), language, complexity, and **advance-directive facts** (`LifeSupportChoice`, `CprDirective`). **The call for Adam:** is that per-document layer, especially the healthcare-directive facts, worth arming for? If yes it is two fields. If no, retiring is 9 `wills-*.ts` files and 4 exports from `index.ts`. |
...
|---|---|---|
| **#162** SA key in git history | Headline key `c059f6a5…` **provably dead** (7/27 GCP-verified session record below); history never rewritten (blob inert, marked optional); two stray user-managed keys still parked for Adam; **no secret scanner or push protection exists anywhere** — that half was tracked nowhere. | **Adam:** confirm the 7/27 record closes the headline; delete the two stray keys (parked item). **Agent:** gitleaks CI step **✅ MERGED 2026-08-07 (#312, Adam's sign-off given in-session)** — pinned checksum-verified binary, full-history scan on every push/PR; `.gitleaks.toml` allowlists the public Firebase web key + test fixture, `.gitleaksignore` pins 11 dead historical fingerprints with rotation evidence; pass and fail paths both verified before shipping. **Adam, still open:** enable GitHub push protection (Settings → Code security). |
| **#166** email callables | All six still take recipients from `request.data`, but `assertStaff()` (T6) closed the client-side path; Adam skipped T9 server-resolve (section I) — that half stands as decided. **Unfixed + untracked:** HTML-escaping gaps at `lawpay-integration.ts:461,467` (clientName/description) and `buildEmailHtml` (`email-notifications.ts:137-147`, firmName/logoUrl raw into every outbound email). | **✅ Agent half DONE 2026-08-07 (#311):** LawPay email escapes clientName/description/branding/URLs; `buildEmailHtml` escapes all branding and http(s)-gates the logo URL; 4 tests written failing-first. Recipient half stands as the recorded T9 skip. |
| **#168** Levitate sync | **✅ Agent half DONE 2026-08-08 (#315 + #320):** URL never reaches any console line on any path — including the vendor's own error body, which the adversarial pass proved could smuggle it back in; payload minimized to name + email + clientId + source; success AND failure audited as `integration_synced`. (Credential *storage* was already fixed pre-issue by #59.) | **Adam:** the consent / default-off gate — an RPC 1.6 disclosure question, same territory as the clause-corpus stance. |
...
| **#170** portal trust boundary | **✅ Agent half DONE 2026-08-08 (#320):** 14-day token TTL (fail-closed on any malformed stamp; expired indistinguishable from unknown), re-mint on expiry, `rotate: true` revocation via `clientService.getRegistrationLink`, `client_registered` audit on both registration doors — adversarially verified incl. type-confusion probes. | **Agent + Adam sign-off (Never-Break), still open:** the rules self-update field allowlist (`firestore.rules` — a client can still write arbitrary fields on their own record, including `registrationToken`; the idiom exists at `:297-300` for users). Urgency rises when E1/E2 add a second public write path. |
...
| **#174** compliance & lifecycle | No client-deletion cascade or retention/TTL paths anywhere; no DPA register; ~~e-sign defaults to test mode with no UI banner~~ **✅ banner SHIPPED 2026-08-08 (#320)** — shown BEFORE sending, mirroring the server default, fail-closed on a failed read; per-firm `lawpayPaymentPageUrl` exists (partial credit). No App Check (`enforceAppCheck` appears nowhere). | **Adam:** retention schedule, DPA register, e-sign default policy. **Agent after:** `deleteClientData` cascade + TTLs once the schedule exists. |
...
### ▶ CONTINUATION, same day — Adam: "Merge everything and keep going"

**D1's sign-off is RECORDED: Adam replied "Merge everything and keep going" directly to the
session summary that named #318 as held and why** — a directed answer to the sign-off request,
not a stray approval. #318 merged (`b585a2a`); hosting run #124 deployed it ✅. The queue then
continued:

- **#168 agent half (in #320):** webhook-URL redaction on every path, payload minimized to the
  CRM contact card (name + email + clientId + source, per the issue's own spec), failed pushes
  audited. The adversarial pass **broke the first redaction** — the HTTP-failure path still
  logged `response.text()`, and Zapier/Make error bodies routinely echo the host or full secret
  URL — fixed, with the hostile-body case pinned. **Adam's half unchanged:** the consent /
  default-off gate (RPC 1.6 territory).
- **#170 agent half (in #320):** 14-day TTL on registration tokens (fail-closed on any
  malformed stamp; expired == unknown, no oracle), re-mint on expiry, `rotate: true` as the
  revocation path, `client_registered` audit entries on both registration doors. The verifier
  confirmed the TTL logic fails closed on every type-confusion probe. **Still proposed
  (Never-Break):** the client self-update field allowlist in firestore.rules — a client can
  still write arbitrary fields on their own record, including `registrationToken`.
- **#174 (in #320):** the e-sign TEST MODE banner now shows BEFORE sending (mirrors the server
  default exactly; fail-closed on a failed firm-doc read). Retention schedule, DPA register and
  the e-sign default policy remain Adam's.
- **D2 (in #320):** generation reads `interviewSettings/current → trust.apportionmentMode` via
  the new `interview-settings.ts` bridge (the single mapping point for future D-sections), at
  both firm-load sites. Precedence: interview setting > legacy firm field > 'hybrid' code
  default. #280's loose end is closed — the mode is now a firm default the Settings → Interview
  screen actually controls.
