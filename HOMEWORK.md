...
**▶️ Next actionable work (pick from these — no Round 5 highs left):** (1) the broken **Card charge / AffiniPay Hosted Fields** flow (🔴 section below — needs a live Adam test loop); (2) the **`linkClient` email-claim sibling** flagged above; (3) the **CI functions-deploy root-cause** fix (issue #64, 🚩 section below); (4) Round 5 **mediums** if any remain in `docs/AUDIT-findings.md`.
