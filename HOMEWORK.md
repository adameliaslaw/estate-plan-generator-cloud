...
**▶️ NEXT (this session): R5-037 systemic rules fix + emulator test.** `firestore.rules` `isAdmin()` = bare `hasRole('admin')` with no firm scoping, OR'd into ~40 firm-scoped match blocks → any admin can read/write EVERY firm's data (clients, documents, notes, payments, pendingTranscripts, emailTemplates…). Plan: add `isFirmAdmin(firmId) = isAdmin() && belongsToFirm(firmId)`, replace the firm-scoped `isAdmin()` sites with `isFirmAdmin(firmId)` (leave any genuinely global/top-level admin uses — audit each), then run the security-rules emulator tests before merge. Never-Break rules change → separate PR, emulator-verified. **Do NOT touch the top-level `firms/{firmId}` doc rules without checking they still let an admin manage their own firm.**
